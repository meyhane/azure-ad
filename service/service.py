...
from dao_helper import init_dao, get_all_objects, init_dao_on_behalf_on, stream_as_json, make_request, GRAPH_URL
...
def generic_graph_api_request(path=None):
    ...
    :return: 200 for i)all GET requests even if there was an error ii)any other successful request
             500 for other any failed request
    """
    if r.args.get('auth') and r.args.get('auth') == 'user':
        init_dao_on_behalf_on(env('client_id'), env('client_secret'), env('tenant_id'), env('username'),
                              env('password'))
